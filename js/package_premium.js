    ...
    const timelineObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
               ...
               // SOSTITUISCI la funzione initTimelineAnimation() nel tuo package_premium.js con questa:
...
function initTimelineAnimation() {
    ...
    // Detecta se è mobile per soglie diverse
    const isMobile = window.innerWidth <= 768;
    
    const timelineObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                // SOLUZIONE: Una sola classe CSS invece di style changes
                entry.target.classList.add('timeline-animate-in');
                
                // SOLUZIONE: Un solo requestAnimationFrame invece di setTimeout multipli
                requestAnimationFrame(() => {
                    const content = entry.target.querySelector('.timeline-content');
                    const marker = entry.target.querySelector('.timeline-marker');
                    
                    if (content) {
                        content.classList.add('content-slide-in'); // Solo classe CSS
                    }
                    
                    if (marker) {
                        marker.classList.add('marker-pulse'); // Solo classe CSS
                    }
                });
        ...
        threshold: isMobile ? 0.2 : 0.4,    // Soglia più bassa su mobile
        rootMargin: isMobile ? '-20px' : '-50px'  // Margine ridotto su mobile
    });
